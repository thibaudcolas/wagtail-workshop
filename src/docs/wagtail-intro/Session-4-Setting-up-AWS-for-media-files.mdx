...
In the AWS console locate the [IAM user management](https://console.aws.amazon.com/iam/home) from the main menu

![as](./aws_2.png)

Click Users > then Add user

I'm going to call my user `codersofcolouruser`

![](./aws_3.png)

Next you will be asked to add the user to a group. For now, we'll set up a full S3 user. Click "Create group"

![](./aws_4.png)

Group Name: S3

In the policies, search for s3 and find "AmazonS3FullAccess", tick it, then search for iam, and tick "IAMFullAccess", and click "Create group".

When your group is created, it’s automatically assigned to the user we created. Click "Next: Tags", bottom right
...
Now we can run buckup. So in our terminal, run `buckup --region eu-west-2`

```bash
buckup --region eu-west-2
```
